...
import { ENGLISH_CATEGORY_ORDER, DIFFICULTY_LABELS, DIFFICULTY_TONES } from "@/lib/plan";
...
export default function ForYouPage() {
  ...
  const [startingSkill, setStartingSkill] = useState<QuestionSkill | null>(null);
  ...
  useEffect(() => {
    async function load() {
      ...
      const byTier: Partial<Record<QuestionSkill, Partial<Record<Difficulty, TierStats>>>> = {};
      const perSessionSkillCounts: Record<string, Partial<Record<QuestionSkill, number>>> = {};
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      (answersRes.data ?? []).forEach((row: any) => {
        const q = Array.isArray(row.question) ? row.question[0] : row.question;
        if (!q) return;
        const skill = q.skill as QuestionSkill;
      ...
      const tierRank: Record<Difficulty, number> = { easy: 0, "medium-low": 1, "medium-high": 2, hard: 3 };

      const allSkills = Object.values(DOMAIN_SKILLS).flat();
      const built: Record<string, SkillInsight> = {};
      ...
      allSkills.forEach((skill) => {
        ...
        if (progressTiers.length > 0) {
          currentTier = progressTiers.sort((a, b) => tierRank[b] - tierRank[a])[0];
        } else if (hasData) {
        ...
        built[skill] = {
          skill, label: SKILL_LABELS[skill], note: SKILL_NOTES[skill], hasData,
      ...
      setInsights(built as Record<QuestionSkill, SkillInsight>);
      ...
      if (sessions.length > 0) {
        ...
        recent.forEach((s) => {
          ...
          Object.entries(counts).forEach(([sk, n]) => {
            const label = SKILL_LABELS[sk as QuestionSkill] ?? sk;
  ...
  const allSkills = Object.values(insights);
  const withData = allSkills.filter((s) => s.hasData);
  ...
  const overallCorrectPct = Math.round(
    (withData.reduce((a, s) => a + (s.accuracy * s.totalAnswered) / 100, 0) / withData.reduce((a, s) => a + s.totalAnswered, 0)) * 100
  );
  const overallGraspN = Math.round(withData.reduce((a, s) => a + s.graspN, 0) / withData.length);

  const byWeakness = (a: SkillInsight, b: SkillInsight) => a.graspN - b.graspN || a.accuracy - b.accuracy;
  const weakest = withData.slice().sort(byWeakness).slice(0, 3);

  const domainNames = Object.keys(DOMAIN_SKILLS) as Domain[];
  const domainData = domainNames.map((name) => {
    const skills = DOMAIN_SKILLS[name].map((sk) => insights[sk]);
  ...
  const sel = domainData.find((d) => d.name === selected)!;
  const selMeta = DOMAIN_ICON[sel.name];
    ...
    <div style={{ minHeight: "100vh", background: "var(--canvas)" }}>
      ...
      <main style={{ maxWidth: 1040, margin: "0 auto", padding: "40px 24px 64px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 14, marginBottom: 30 }}>
          {domainData.map((d) => {
            const meta = DOMAIN_ICON[d.name];
            const isSel = d.name === selected;
            return (
              <button key={d.name} onClick={() => setSelected(d.name)} style={{
